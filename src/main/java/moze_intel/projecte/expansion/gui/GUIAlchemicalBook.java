...
public class GUIAlchemicalBook extends Screen {
	...
	protected void init() {
		...
		//26.3: Level#getBiome returns a Holder, so the biome name comes off the holder's ResourceKey
		// instead of the (removed) Registry#getKey(Holder)
		String biomeName = player.level().getBiome(player.getOnPos())
				.unwrapKey()
	...
	public Component getTeleportationTooltip(CapabilityAlchemicalBookLocations.TeleportLocation location, boolean canTeleport) {
		...
		if(canTeleport) {
			...
			//26.3: ResourceKey#location() was renamed to #identifier()
			tooltips.add(Lang.GUI.ALCHEMICAL_BOOK_DIMENSION.translate(Component.translatable(location.dimension().identifier().toString())));
		}
