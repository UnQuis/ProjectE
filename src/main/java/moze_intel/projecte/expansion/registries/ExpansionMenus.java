...
public class ExpansionMenus {
	 ...
	 * {@link INamedEntry}, while ProjectExpansion keyed its containers off plain names ({@code collector_tier_1} rather than
	 * {@code collector}), so a tiny name holder is built here. The block entity lookup itself is delegated to ProjectE's
	 * helper so that this class keeps free of client only classes.
	 */
	public static <CONTAINER extends ContainerBase, BE extends BlockEntity> ContainerTypeRegistryObject<CONTAINER> registerBlockEntity(String name, Class<BE> blockEntityClass, ContainerTypeDeferredRegister.IBlockEntityContainerFactory<CONTAINER, BE> factory) {
		return MENUS.register(new NameEntry(name), blockEntityClass, factory);
	}

	private record NameEntry(String name) implements INamedEntry {
		@Override
