...
import moze_intel.projecte.expansion.registries.ExpansionAdvancements;
 ...
 * {@link ServerTickEvent.Post} is a game bus event so nothing has to be declared.
 */
...
public class ServerEvents {
	...
	private static void handleWeighedDown(@SuppressWarnings("unused") ServerTickEvent.Post event, ServerPlayer player) {
		...
		Item compactSun = ExpansionBlocks.COMPACT_SUN.asItem();
		boolean hasCompactSun = player.isHolding(compactSun);

		if (protectionAmount >= 100) return;

		if (!hasCompactSun) {
			//26.3: Inventory#items is no longer public, getNonEquipmentItems() is the accessor PE itself uses
			for (ItemStack stack : player.getInventory().getNonEquipmentItems()) {
				if (stack.getItem() == compactSun) {
					hasCompactSun = true;
					break;
				}
			}
		...
		if (hasCompactSun) {
			//26.3: the MobEffects constant for the "slowness" effect was renamed to SLOWNESS
			player.addEffect(new MobEffectInstance(MobEffects.SLOWNESS, 5, 3 - protectionLevel, false, false));
		}
	}
	...
	private static void handleSunExposure(ServerTickEvent.Post event, ServerPlayer player) {
		...
		ExpansionDamageSources damage = ExpansionDamageSources.fromServer(event.getServer());
			...
			if (block instanceof BlockCompactSun) {
				...
				if (timer.over()) {
					if (protectionAmount < 100) {
						...
						AdvancementHolder advancement = event.getServer().getAdvancements().get(ExpansionAdvancements.BLINDED_BY_THE_LIGHT);
						if (advancement != null) {
