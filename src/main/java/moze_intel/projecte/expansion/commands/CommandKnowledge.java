...
public class CommandKnowledge {
	...
	private static int handle(CommandContext<CommandSourceStack> ctx, ActionType action) throws CommandSyntaxException {
		...
		//26.3: ItemArgument#getItem now hands back an ItemInput, the Item has to be built from it
		//(same call as ProjectE's own KnowledgeCMD#handle)
		...
		if (response == 1 && action != ActionType.TEST) {
			provider.syncKnowledgeChange(player, IEMCProxy.INSTANCE.getPersistentInfo(ItemInfo.fromItem(item)), true);
		}
