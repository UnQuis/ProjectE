...
public abstract class AbstractCollectorScreen<T extends CollectorMK1Container> extends PEContainerScreen<T> {
	...
	public void extractBackground(@NotNull GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTicks) {
		...
		graphics.blit(RenderPipelines.GUI_TEXTURED, getTexture(), leftPos + 64 + getBonusXShift(), topPos + 18, 0, 166, (int) ((double) menu.emc.get() / menu.collector.getMaximumEmc() * 48), 10, 256, 256);
